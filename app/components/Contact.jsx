      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-5 my-3'>
        {socialMedia.map(({ name, icon, color, link, account, massage }, index) => {
          ...
          <div
            ...
            key={index}
          >
            <div class="flex items-center gap-3 gap-y-3">
                <a href={link}><FontAwesomeIcon icon={icon} style={{ color: color }} className="text-6xl" /></a>
                <div class="block gap-y-3">
                    <a href={link}><h2 class="font-bold text-xl">{name}</h2></a>
                    <a href={link}><p class="text-md">{account}</p></a>
                    <a href={link} target="_blank" class="text-blue-500 font-semibold">{massage}</a>
                </div>
            </div>
          </div>
        )})}
